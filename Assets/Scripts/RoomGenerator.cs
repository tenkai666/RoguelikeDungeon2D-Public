...
public class RoomGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        rooms[0].GetComponent<SpriteRenderer>().color = startColour;//改变第1个房间的颜色
        //rooms[roomNumber-1].GetComponent<SpriteRenderer>().color = endColour;
        ...
        endRoom.GetComponent<SpriteRenderer>().color = endColour;
    }
    ...
    {
        //检查碰撞体是否位于一个圆形区域内,判断上下左右是否有其他房间
    ...
    public void FindEndRoom()
    {
        ...
        for (int i = 0; i < lessFarRooms.Count; i++)
        {
            if (lessFarRooms[i].GetComponent<Room>().doorNumber == 1)
                oneWayRooms.Add(lessFarRooms[i]);//第二远房间里的单侧门加入
        }
